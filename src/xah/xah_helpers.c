...
static size_t xah_encode_base58_address(const base58_buf_t *in, xah_address_t *out) {
...
void xah_public_key_hash160(xah_pubkey_t *pubkey, uint8_t *out) {
    union {
        cx_sha256_t shasha;
...
size_t xah_public_key_to_encoded_base58(xah_pubkey_t *pubkey,
    ...
    if (pubkey != NULL) {
        xah_public_key_hash160(pubkey, tmp.buf + version_size);
    }
    if (account != NULL) {
